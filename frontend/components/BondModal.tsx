...
export default function BondModal({ bond, onClose, onUpdate }: BondModalProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Не удалось удалить облигации');
      }
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetchWithAuth(`/bonds/${bond.bond_id}/specification`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleChange = (field: keyof typeof editedBond, value: string | number) => {
    setEditedBond(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleChangeQuantity = async (isPositive: boolean) => {
    ...
    if (!isPositive && quantityToChange > bond.quantity) {
      setQuantityError(`Невозможно уменьшить на ${quantityToChange}. У вас только ${bond.quantity} шт.`);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || 'Не удалось изменить количество');
      }
      ...
      setQuantityError(err instanceof Error ? err.message : 'Ошибка изменения количества');
    } finally {
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                {isEditing ? (
                  ...
                  <p className="text-xl font-bold text-gray-900">{bond.series}</p>
                )}
              </div>

              <div className="bg-blue-50 p-4 rounded-lg">
                <p className="text-sm text-gray-500 mb-2">Количество</p>

                {!showQuantityInput ? (
                  <div className="flex items-center gap-2">
                    <p className="text-xl font-bold text-blue-600">{bond.quantity} шт.</p>
                    <button
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={(e) => handleChange('nominal_value', parseFloat(e.target.value))}
              ...
              <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                <span className="text-gray-600">Начальная процентная ставка</span>
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      step="0.01"
                      value={editedBond.initial_interest_rate}
                      onChange={(e) => handleChange('initial_interest_rate', parseFloat(e.target.value))}
                ...
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      step="0.01"
                      value={editedBond.reference_rate_margin}
                      onChange={(e) => handleChange('reference_rate_margin', parseFloat(e.target.value))}
                  ...
                  <span className="font-semibold text-gray-900">
                    {bond.reference_rate_margin > 0 ? '+' : ''}{bond.reference_rate_margin}%
                  </span>
                )}
              </div>

              <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                <span className="text-gray-600">Первый процентный период</span>
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    <input
                      type="number"
                      value={editedBond.first_interest_period}
                      onChange={(e) => handleChange('first_interest_period', parseInt(e.target.value))}
                      className="font-semibold text-gray-900 w-24 bg-white border border-gray-300 rounded px-2 py-1 text-right"
                    />
                    <span className="font-semibold text-gray-900">мес.</span>
                  </div>
                ) : (
                  <span className="font-semibold text-gray-900">
            ...
            <div className="space-y-3">
                ...
                {isEditing ? (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      onChange={(e) => handleChange('maturity_period', parseInt(e.target.value))}
                      className="font-semibold text-gray-900 w-24 bg-white border border-gray-300 rounded px-2 py-1 text-right"
                    />
                    <span className="font-semibold text-gray-900">мес.</span>
                  </div>
              ...
              {bond.last_update && (
                <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                  ...
                  <span className="font-semibold text-gray-900">
                    {new Date(bond.last_update).toLocaleString('ru-RU', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                      hour: '2-digit',
          ...
          <div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg space-y-2">
              <div className="flex justify-between">
                <span className="text-gray-600">Общая стоимость портфеля</span>
                <span className="font-bold text-gray-900 text-lg">
                  {((isEditing ? editedBond.nominal_value : bond.nominal_value) * bond.quantity).toLocaleString()} ₽
                </span>
              </div>

              <div className="flex justify-between">
                <span className="text-gray-600">Годовой доход (ожидаемый)</span>
                <span className="font-bold text-green-600 text-lg">
                  {(((isEditing ? editedBond.nominal_value : bond.nominal_value) * bond.quantity * (isEditing ? editedBond.initial_interest_rate : bond.initial_interest_rate)) / 100).toLocaleString()} ₽
                </span>
              </div>
            </div>
          </div>
