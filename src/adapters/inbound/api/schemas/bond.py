...
class BondBase(BaseModel):
    ...
    first_interest_period: int = Field(..., ge=0, description="First interest period")
    reference_rate_margin: float = Field(..., ge=0, description="Reference rate margin")
...
class BondUpdateRequest(BaseModel):
    ...
    first_interest_period: int | None = Field(None, ge=0)
    reference_rate_margin: float | None = Field(None, ge=0)
